...
int StackInit(Stack_t** stk, size_t InitCapacity, size_t ElemSize)
{
    ...
    *stk = (Stack_t*)calloc(1, sizeof(Stack_t));
    ...
    {
        if(((*stk)->data = calloc(InitCapacity, ElemSize)) == NULL)
        {
            fprintf(stderr, "Dynamic Memory dead\n");
            (*stk)->Error += ALLOC_ERROR;
            return ALLOC_ERROR; 
...
int StackPush(Stack_t* stk, void* elem)
{
    ...
    memcpy((char*)stk->data + (stk->size)*stk->elSize, elem, stk->elSize);
    stk->size++;
...
int StackPop(Stack_t* stk, void* elem)
{
    ...
    memcpy(elem, (char*)stk->data + (stk->size - 1)*stk->elSize, stk->elSize);
...
int FStackPop(Stack_t* stk)
{
    ...
    memcpy((char*)stk->data + (stk->size - 1) * stk->elSize, POISON_PTR, stk->elSize);
...
int StackLook(Stack_t* stk, void* dest)
{
    memcpy(dest, (char*)stk->data + (stk->size - 1) * stk->elSize, stk->elSize);
    return 0;
}
